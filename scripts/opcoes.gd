...
func _on_master_vol_value_changed(value: float) -> void:
	AudioServer.set_bus_volume_db(GlobalScript.master_bus, linear_to_db(value/100.0))
	GlobalScript.valor_slider_master = value


func _on_musica_vol_value_changed(value: float) -> void:
	AudioServer.set_bus_volume_db(GlobalScript.musica_bus, linear_to_db(value/100.0))
